...
yLadder = handles.myData.Dataset{n}{4};
x = 1:length(yOH);
...
% Plot fit sum
if FITSUM, plot(x./120, gaussianXYWBsl([x0;y0;w0],bsl,x), 'r'); end

% Plot experimental data
if OH, plot(x./120, yOH, 'k'); end

% Plot blue dots
if PKMAX, plot(x0./120, yInterp, 'bo', 'MarkerSize', 4, 'MarkerFaceColor', 'b'); end

% Plot residue numbers/labels
if PKNUMS
    for i=pkIdx(1):1:pkIdx(2)
        if exist('seq','var') && ~isempty(seq)
            text(x0(i)./120, yInterp(i)+800, [seq(i), num2str(i)], 'FontSize', 8, 'HorizontalAlignment', 'center');
        else
            text(x0(i)./120, yInterp(i)+800, num2str(i), 'FontSize', 8, 'HorizontalAlignment', 'center');
        end
    end
end
